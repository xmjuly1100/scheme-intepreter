...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            applied = operator.apply_macro(rest, env)
            return scheme_eval(applied, env, True)
...
def eval_all(expressions, env):
    ...
    while curr_expr.second is not nil:
        scheme_eval(curr_expr.first, env)
        curr_expr = curr_expr.second
    last_exp = curr_expr.first
    return scheme_eval(last_exp, env, True)
    # END PROBLEM 8
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        return child_frame
        #except formals.first is nil and vals.first is not nil:
        #    raise SchemeError
        #except formal.
        # END PROBLEM 11
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        #For example in Suite 2 Case 1, the operator of a call expression is another call expression.
        #That's why we need line 216 & 217
        return child_frame
...
def do_and_form(expressions, env):
    ...
    while curr_exp.second is not nil:
        value = scheme_eval(curr_exp.first, env)
        if value is False:
            return False
        curr_exp = curr_exp.second
    return scheme_eval(curr_exp.first, env, True)
    # END PROBLEM 13
